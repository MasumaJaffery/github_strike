...
/**
 * Creates SVG wrapper following github-readme-streak-stats pattern exactly
 * This pattern is proven to work reliably on GitHub profiles
 */
export function createSvgWrapper(
  ...
  const { borderRadius = 4.5, showBorder = true, title = 'GitHub Strike Card' } = options;

  const borderColor = showBorder ? theme.border : 'transparent';

  // Match github-readme-streak-stats format exactly
  return `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' style='isolation: isolate' viewBox='0 0 ${width} ${height}' width='${width}px' height='${height}px'>
    ...
    <g clip-path='url(#outer_rectangle)'>
      <rect stroke='${borderColor}' fill='${theme.background}' rx='${borderRadius}' x='0.5' y='0.5' width='${width - 1}' height='${height - 1}'/>
      ${content}
    </g>
  </svg>`;
}
...
/**
 * Creates text element following streak-stats pattern
 * Key: stroke-width='0' stroke='none' prevents text distortion
 */
export function createText(
  ...
  // Match streak-stats text format exactly - stroke-width='0' stroke='none' is critical
  return `<text x='${x}' y='${y}' stroke-width='0' text-anchor='${anchor}' fill='${color}' stroke='none' font-family='"Segoe UI", Ubuntu, sans-serif' font-weight='${fontWeight}' font-size='${size}px' font-style='normal'>${escapeHtml(text)}</text>`;
}
