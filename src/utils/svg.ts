...
export function createSvgWrapper(
  ...
  const borderAttr = showBorder ? `stroke='${theme.border}' stroke-width='1'` : '';

  // Match github-readme-streak-stats structure - single quotes, inline, simple
  return `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' style='isolation:isolate' viewBox='0 0 ${width} ${height}' width='${width}px' height='${height}px'><title>${escapeHtml(title)}</title><defs><clipPath id='outer_rectangle'><rect width='${width}' height='${height}' rx='${borderRadius}'/></clipPath></defs><g clip-path='url(#outer_rectangle)'><rect fill='${theme.background}' rx='${borderRadius}' x='0.5' y='0.5' width='${width - 1}' height='${height - 1}' ${borderAttr}/>${content}</g></svg>`;
}
...
export function createProgressBar(
  ...
  return `<g><rect x='${x}' y='${y}' width='${width}' height='${height}' rx='${height / 2}' fill='${theme.progressBackground}'/><rect x='${x}' y='${y}' width='${filledWidth}' height='${height}' rx='${height / 2}' fill='${theme.accent}'/></g>`;
}
...
export function createText(
  ...
  return `<text x='${x}' y='${y}' fill='${color}' font-size='${size}px' font-weight='${weight}' font-family='"Segoe UI", Ubuntu, sans-serif' text-anchor='${anchor}'>${escapeHtml(text)}</text>`;
}
